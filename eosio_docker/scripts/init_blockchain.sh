...
echo "=== setup wallet: treechainwal ==="
# key for eosio account and export the generated password to a file for unlocking wallet later
./cleos wallet create -n treechainwal | tail -1 | sed -e 's/^"//' -e 's/"$//' > treechain_wallet_password.txt
# Owner key for treechainwal wallet
./cleos wallet import -n treechainwal --private-key 5JpWT4ehouB2FF9aCfdfnZ5AwbQbTtHBAwebRXt94FmjyhXwL4K
# Active key for treechainwal wallet
./cleos wallet import -n treechainwal --private-key 5JD9AGTuTeD5BXZwGQ5AtwBqHK21aHmYnTetHgk1B3pjj7krT8N

# * Replace "treechainwal" by your own wallet name when you start your own project

# create account for treechainacc with above wallet's public keys
./cleos create account eosio treechainacc EOS6PUh9rs7eddJNzqgqDx1QrspSHLRxLMcRdwHZZRL4tpbtvia5B EOS8BCgapgYA2L4LJfCzekzeSr3rzgSTUXRXwNi8bNRoz31D14en9

# * Replace "treechainacc" by your own account name when you start your own project
...
./scripts/deploy_contract.sh treechain treechainacc treechainwal $(cat treechain_wallet_password.txt)
